...
def apply_renames(asm_file, procedures, dry_run=False):
    ...
    for proc in procedures:
        ...
        # 4a. Replace direct jsr calls: jsr sub_XXXXX (without parentheses)
        jsr_direct_pattern = re.compile(r'\bjsr\s+' + re.escape(old_name) + r'\b')
        content = jsr_direct_pattern.sub(f'jsr {new_name}', content)

        # 4b. Replace direct bsr calls: bsr sub_XXXXX (without .w/.s suffix)
        bsr_direct_pattern = re.compile(r'\bbsr\s+' + re.escape(old_name) + r'\b')
        content = bsr_direct_pattern.sub(f'bsr {new_name}', content)
        ...
        # 7a. Replace data references: dc.w sub_XXXXX (word-sized pointers)
        dcw_pattern = re.compile(r'\bdc\.w\s+' + re.escape(old_name) + r'\b')
        content = dcw_pattern.sub(f'dc.w {new_name}', content)

        # 7b. Replace lea (sub_XXXXX).l,aX (load effective address)
        lea_pattern = re.compile(r'\blea\s+\(' + re.escape(old_name) + r'\)\.l\s*,')
        content = lea_pattern.sub(f'lea ({new_name}).l,', content)
