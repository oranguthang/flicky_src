...
DEFINITION_RE = re.compile(r"^([A-Za-z_][A-Za-z0-9_]*)(?=:|\s+equ\b|\s+macro\b)", re.IGNORECASE)
...
def load_renames(path: Path) -> list[tuple[str, str]]:
    """Read old,new pairs from a CSV. A header row is optional."""
...
def apply_renames(
    ...
    for path in files:
        ...
        for index, line in enumerate(lines):
            ...
            new_line, count = pattern.subn(lambda m: mapping[m.group(1)], line)
            replaced += count
...
def main() -> int:
    ...
    parser.add_argument(
        "--no-provenance",
        action="store_true",
        help="Do not append '; was: <old>' to the definition line",
    )
    ...
    print(f"[OK] renamed {len(renames)} symbols, {replaced} references, {annotated} annotated")
