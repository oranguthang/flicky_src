...
Sys_GameEntryPoint:
                ...
                bsr.w   Gfx_LoadTilesToVRAM
                lea     (Ram_GroundTilePtr).w,a6
                moveq   #0,d7
                move.w  #$1FF,d6

Sys_GameEntryPoint_ClearObjectsLoop:
                move.l  d7,(a6)+
...
Sys_ModeLoadSegaScreen:
                jmp     Sys_LoadSegaScreen
...
Sys_InitTitleScreen:
                jsr     j_Sys_InitGameState
                lea     (Ram_SpriteTableCursor).w,a6
                moveq   #0,d7
                move.w  #$1FF,d6

Sys_InitTitleScreen_ClearLoop:
                move.l  d7,(a6)+
                dbf     d6,Sys_InitTitleScreen_ClearLoop
                bsr.w   Camera_ClearScroll
                bsr.w   Object_ClearAllSlots
                move.w  #$8000,(Ram_TextTileBase).w
                jmp     Gfx_LoadLogoAndExitToVRAM

Gfx_LoadTilesToVRAM:
                move.w  #$200,d0
                jsr     j_Gfx_SetTileWriteAddr
                lea     (Data_LevelTiles).l,a0
                jsr     j_Nem_Decomp
                move.w  #$400,d0
                jsr     j_Gfx_SetTileWriteAddr
                lea     (Data_SpritesTiles).l,a0
                jsr     j_Nem_Decomp
                move.b  #1,(Ram_FontBankFlag).w

Gfx_LoadTilesToVRAM_LoadFont:
                moveq   #$20,d0
                lea     (VDP_CTRL).l,a6
                jsr     j_Gfx_SetTileWriteAddr
                lea     (Data_Jap1BPPTiles).w,a0
                moveq   #$20,d0
...
Gfx_LoadLogoAndExitToVRAM:
                lea     (VDP_CTRL).l,a6
                move.w  #$640,d0
                jsr     j_Gfx_SetTileWriteAddr
                lea     (Data_ScoresTiles).l,a0
                jsr     j_Nem_Decomp
                move.w  #$693,d0
                jsr     j_Gfx_SetTileWriteAddr
                lea     (Data_ExitTiles).l,a0
                jsr     j_Nem_Decomp
                rts
